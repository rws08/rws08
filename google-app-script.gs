...
// 주석의 위치
var COLUMN_INDEX_COMMENT = 0;

// 데이터 위치
var COLUMN_INDEX_KEY = 5;
var COLUMN_INDEX_VALUE_START = 6;
var COLUMN_INDEX_VALUE_END = 7;
var ROW_INDEX_VLAUE = 4;
...
function exportResources() {
    ...
    exportFolder = createOrGetFolder("Export", parentFolder);
    appExportFolder = createOrGetFolder(APP_NAME, exportFolder);
...
function createAndroidResources(language, data, folder, column) {
    ...
    for (var i = ROW_INDEX_VLAUE; i < data.length; i++) {
...
function downloadFile(){
  ...
  zipFolder(appExportFolder, 'strings.zip', exportFolder)

  var file = exportFolder.getFilesByName('strings.zip').next();
  var output = HtmlService.createHtmlOutput("<div>");
  output.append("<a target=\"_blank\" href=\"https://drive.google.com/uc?export=download&id=");
  output.append(file.getId());
  output.append("\"> download </a>");
  output.append("</div>");
  output.setTitle('Download');
  SpreadsheetApp.getActive().show(output);
}
...
function zipFolder(pathFolder, filename, destinyPath) {
  var files = destinyPath.getFilesByName(filename)
  if (files.hasNext()) {
    var file = destinyPath.getFilesByName(filename).next();
    file.setTrashed(true)
  }

  var zip = Utilities.zip(getBlobsPath(pathFolder, ''), filename);
  destinyPath.createFile(zip);
}
